...
def get_unet(hyperparameters):
    ...
    if hyperparameters.get('use_weight_maps'):
        ...
        loss = hyperparameters['loss_func'](weight_input, hyperparameters['weight_strength'])
    else:
        model = Model(inputs=[inputs], outputs=[conv10])
        loss = hyperparameters['loss_func']
...
def load_saved_model(hyperparameters):
    model_path = os.path.join(os.path.join(os.getcwd(), 'models'), hyperparameters['use_model'])
    if not os.path.isfile(model_path):
        raise ValueError("Model name is not correct")

    custom_objects = {}
    custom_objects[hyperparameters['loss']] = hyperparameters['loss_func']
    for i, metric in enumerate(hyperparameters['metrics']):
        custom_objects[metric] = hyperparameters['metrics_func'][i]
